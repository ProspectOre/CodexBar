...
3. A real app-Auto harness (`CodexBarCLI usage --app-auto-verifier`), isolated from the user's CodexBar account
   configuration executes the same app provider runtime while retaining machine-readable verifier output. The harness
   preserves the user's stored prompt policy, temporarily sets the explicit `.always` opt-in required for a background
   owner probe, and restores the exact prior value before evaluating the result. It must observe direct OAuth absence
   and then return a Claude/CLI usage payload through the logged-in owner CLI within a fixed deadline. A usable
   CodexBar-owned direct OAuth credential therefore makes this fallback-specific proof fail closed. The verifier records
   the helper's process tree and rejects any `/usr/bin/security` descendant outside the installed Claude owner's
   subtree. Process names are normalized for macOS's parenthesized zombie rendering before classification. Claude-owned
   MCP tools remain recorded
   but are not misattributed to CodexBar. A public OSLog canary first proves visibility of the bounded log window; the audit
   retains noninteractive Security.framework activity from CodexBar-owned cache lookups separately, then requires zero
   prompt or authorization UI signals attributable to CodexBarCLI across securityd, coreauthd, authd, SecurityAgent,
   authorizationhost, CoreServicesUIAgent, and UserNotificationCenter. This is positive route-liveness evidence; the
   tests and Release artifact audit carry the negative proof that no foreign reader exists.

The verifier shell performs no Keychain query. The packaged candidate may perform noninteractive reads of its own OAuth
cache, and the visibility canary emits only a public OSLog marker. The verifier invokes the installed Claude CLI against
the live profile; as credential owner, Claude may read or refresh its own Keychain item. Run it only with explicit
consent.
...
artifacts come from the source under test. The fallback-specific live phase also requires the active profile to have no
usable CodexBar-owned direct OAuth credential. It leaves a mode-0700 temporary report directory containing the package
and test logs, Release string audits, process-tree snapshots, CLI output, and bounded unified-log evidence. Treat that
