...
- Package locally: `./Scripts/package_app.sh` to refresh `CodexBar.app`, then restart with `pkill -x CodexBar || pkill -f CodexBar.app || true; cd "$(git rev-parse --show-toplevel)" && open -n CodexBar.app`.
