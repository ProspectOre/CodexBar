...
- Package locally: `./Scripts/package_app.sh` to refresh `CodexBar.app`, then restart the freshly built bundle from this checkout.
...
- To guarantee the right bundle is running after a rebuild, stop the old process and open the freshly built `CodexBar.app` from this checkout.
