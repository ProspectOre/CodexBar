...
public enum ClaudeOAuthRefreshFailureGate {
        ...
        now: Date) -> Bool
    {
        ...
        // Unscoped failure state was written by builds that always used the fixed
        // ~/.claude/.credentials.json profile. A custom launch environment must not claim or clear it.
        let isDefaultProfile = profileIdentifier ==
