...
struct ClaudeOAuthCredentialsStoreTests {
    ...
    @Test
    func `load record never repairs a cache miss from ambient keychain data`() throws {
        let service = "com.steipete.codexbar.cache.tests.\(UUID().uuidString)"
        try KeychainCacheStore.withServiceOverrideForTesting(service) {
            KeychainCacheStore.setTestStoreForTesting(true)
            defer { KeychainCacheStore.setTestStoreForTesting(false) }
    ...
    @Test
    func `unattributed keychain change does not replace cached credentials`() throws {
        let service = "com.steipete.codexbar.cache.tests.\(UUID().uuidString)"
        try KeychainCacheStore.withServiceOverrideForTesting(service) {
            try KeychainAccessGate.withTaskOverrideForTesting(false) {
                KeychainCacheStore.setTestStoreForTesting(true)
...
extension ClaudeOAuthCredentialsStoreTests {
    ...
    @Test
    func `never mode does not assign a global Keychain item to a missing profile`() throws {
