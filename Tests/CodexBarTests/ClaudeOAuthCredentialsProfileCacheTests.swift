...
struct ClaudeOAuthCredentialsProfileCacheTests {
    ...
    @Test
    func `unattributed global keychain change cannot cross profile cache ownership`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        ...
        fingerprintStore.fingerprint = ClaudeOAuthCredentialsStore.ClaudeKeychainFingerprint(
            modifiedAt: 1,
            createdAt: 1,
            persistentRefHash: "profile-b-before")
        ...
        try self.withIsolatedCache {
            ...
            let globalKeychainData = self.makeCredentialsData(accessToken: "profile-a-refreshed-token")
            let changedFingerprint = ClaudeOAuthCredentialsStore.ClaudeKeychainFingerprint(
                modifiedAt: 2,
                createdAt: 1,
                persistentRefHash: "profile-a-after")
            let record = try ClaudeOAuthKeychainPromptPreference.withTaskOverrideForTesting(.always) {
            ...
            #expect(fingerprintStore.fingerprint?.persistentRefHash == "profile-b-before")

            for (identifier, expectedToken) in [
                (identifierA, "profile-a-token"),
                (identifierB, "profile-b-token"),
            ] {
                switch KeychainCacheStore.load(
                    key: ClaudeOAuthCredentialsStore.cacheKeyForTesting(profileIdentifier: identifier),
                    as: ClaudeOAuthCredentialsStore.CacheEntry.self)
                {
                case let .found(entry):
                    #expect(try ClaudeOAuthCredentials.parse(data: entry.data).accessToken == expectedToken)
    ...
    @Test
    func `default profile invalidation tombstones unmigrated legacy cache`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
                    ...
                    historicalProfileIdentifier)
                {
                    try ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(missingCredentialsURL) {
                        KeychainCacheStore.store(
                            key: legacyKey,
                            entry: LegacyCacheEntry(
                                data: self.makeCredentialsData(accessToken: "unmigrated-legacy-token"),
                                storedAt: Date(),
                                owner: .codexbar,
                                historyOwnerIdentifier: nil))
                        ...
                        do {
                            _ = try ClaudeOAuthCredentialsStore.loadRecord(
                                ...
                                allowKeychainPrompt: false,
                                allowClaudeKeychainRepairWithoutPrompt: false)
                            Issue.record("Expected invalidation to block legacy cache migration")
                        } catch let error as ClaudeOAuthCredentialsError {
                            guard case .notFound = error else {
                                Issue.record("Expected .notFound, got \(error)")
                                return
                        ...
                        guard case .missing = KeychainCacheStore.load(
                            ...
                            Issue.record("Expected the attributed legacy cache to be removed")
                            return
                        }
                    }
                }
    ...
    #if os(macOS)
    @Test
    func `temporary legacy lookup keeps default profile invalidated until ownership recheck`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
                    ...
                    historicalProfileIdentifier)
                {
                    try ClaudeOAuthCredentialsStore.withCredentialsURLOverrideForTesting(missingCredentialsURL) {
                        KeychainCacheStore.store(
                            key: legacyKey,
                            entry: LegacyCacheEntry(
                                data: self.makeCredentialsData(accessToken: "stale-unavailable-legacy-token"),
                                storedAt: Date(),
                                owner: .codexbar,
                                historyOwnerIdentifier: nil))
                        ...
                        do {
                            _ = try ClaudeOAuthCredentialsStore.loadRecord(
                                ...
                                allowKeychainPrompt: false,
                                allowClaudeKeychainRepairWithoutPrompt: false)
                            Issue.record("Expected recovered stale legacy cache to remain invalidated")
                        } catch let error as ClaudeOAuthCredentialsError {
                            guard case .notFound = error else {
                                Issue.record("Expected .notFound, got \(error)")
                                return
                        ...
                        guard case .missing = KeychainCacheStore.load(
                            ...
                            Issue.record("Expected ownership recheck to retire the stale legacy cache")
                            return
                        }
                    }
                }
    ...
    @Test
    func `custom profile invalidation preserves identified sibling legacy cache`() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let environmentA = ["CLAUDE_CONFIG_DIR": root.appendingPathComponent("profile-a").path]
        let environmentB = ["CLAUDE_CONFIG_DIR": root.appendingPathComponent("profile-b").path]
        ...
        try self.withIsolatedCache {
            KeychainCacheStore.store(
                key: legacyKey,
                entry: ClaudeOAuthCredentialsStore.CacheEntry(
                    data: self.makeCredentialsData(accessToken: "profile-a-legacy-token"),
                    storedAt: Date(),
                    owner: .codexbar,
                    profileIdentifier: identifierA))

            ClaudeOAuthCredentialsStore.invalidateCache(environment: environmentB)
            ...
            {
            case let .found(entry):
                #expect(entry.profileIdentifier == identifierA)
                let credentials = try? ClaudeOAuthCredentials.parse(data: entry.data)
                #expect(credentials?.accessToken == "profile-a-legacy-token")
            case .missing, .invalid, .temporarilyUnavailable:
                Issue.record("Expected profile B invalidation to preserve profile A's identified legacy cache")
            }
        }
    }
