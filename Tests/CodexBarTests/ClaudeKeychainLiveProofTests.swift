...
struct ClaudeKeychainLiveProofTests {
    ...
    func `live app Auto transitions missing safe OAuth to the owner CLI`() async throws {
        ...
        let ownerCLIAvailable = await ClaudeOAuthKeychainPromptPreference.withTaskOverrideForTesting(.always) {
            await strategies[1].isAvailable(context)
        }
        #expect(ownerCLIAvailable)
        ...
        let outcome = await ClaudeOAuthKeychainPromptPreference.withTaskOverrideForTesting(.always) {
            await KeychainCacheStore.withServiceOverrideForTesting(
