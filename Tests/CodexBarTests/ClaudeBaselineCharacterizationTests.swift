...
struct ClaudeBaselineCharacterizationTests {
    ...
    func `opted in background app and CLI runtimes use owner mediated auth status before interactive usage`() async throws {
        let settings = ProviderSettingsSnapshot.make(claude: .init(
            usageDataSource: .auto,
            webExtrasEnabled: false,
            cookieSource: .off,
        ...
        let appCLIAvailable = await self.withBackgroundKeychainAccess {
            await ClaudeOAuthKeychainPromptPreference.withTaskOverrideForTesting(.always) {
                await appCLI.isAvailable(appContext)
            }
        }
