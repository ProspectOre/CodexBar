...
PROMPT_DEFAULTS_DOMAIN="com.steipete.codexbar"
PROMPT_DEFAULTS_KEY="claudeOAuthKeychainPromptMode"
PROMPT_OVERRIDE_ACTIVE=0
PROMPT_ORIGINAL_PRESENT=0
PROMPT_ORIGINAL_VALUE=""

restore_prompt_preference() {
  if [[ "$PROMPT_OVERRIDE_ACTIVE" -ne 1 ]]; then
    return
  fi
  if [[ "$PROMPT_ORIGINAL_PRESENT" -eq 1 ]]; then
    /usr/bin/defaults write "$PROMPT_DEFAULTS_DOMAIN" "$PROMPT_DEFAULTS_KEY" \
      -string "$PROMPT_ORIGINAL_VALUE"
  else
    /usr/bin/defaults delete "$PROMPT_DEFAULTS_DOMAIN" "$PROMPT_DEFAULTS_KEY" >/dev/null 2>&1 || true
  fi
  PROMPT_OVERRIDE_ACTIVE=0
}

trap restore_prompt_preference EXIT
...
# Background app Auto may launch the opaque Claude owner probe only after the user explicitly opts in. Preserve the
# user's exact stored preference, opt in for the bounded proof window, and restore it before evaluating the result.
if PROMPT_ORIGINAL_VALUE="$(
  /usr/bin/defaults read "$PROMPT_DEFAULTS_DOMAIN" "$PROMPT_DEFAULTS_KEY" 2>/dev/null
)"; then
  PROMPT_ORIGINAL_PRESENT=1
fi
PROMPT_OVERRIDE_ACTIVE=1
/usr/bin/defaults write "$PROMPT_DEFAULTS_DOMAIN" "$PROMPT_DEFAULTS_KEY" -string always
if [[ "$(/usr/bin/defaults read "$PROMPT_DEFAULTS_DOMAIN" "$PROMPT_DEFAULTS_KEY" 2>/dev/null || true)" != "always" ]]; then
  fail "Could not establish the temporary background owner-CLI prompt policy."
fi
...
ATTRIBUTED_ACTIVITY="$ARTIFACT/codexbar-attributed-keychain-activity.log"
...
rg -i "$ATTRIBUTION_PATTERN" "$UNIFIED_LOG" >"$ATTRIBUTED_ACTIVITY" || true
ATTRIBUTED_ACTIVITY_COUNT="$(wc -l <"$ATTRIBUTED_ACTIVITY" | tr -d ' ')"

# A no-UI Security.framework lookup still creates LAContext and SecItemCopyMatching events. Retain those owned-cache
# events as positive evidence, but classify only explicit prompt/UI signals as authorization failures.
PROMPT_EVENT_PATTERN='QueryKeychainUse|keychain[[:space:]_-]*prompt|SecurityAgent\[|authorizationhost\[|CoreServicesUIAgent\[|UserNotificationCenter\[|(securityd|coreauthd|authd)\[[^]]+\].*(prompt|authoriz|user[[:space:]]+interaction)'
rg -i "$PROMPT_EVENT_PATTERN" "$ATTRIBUTED_ACTIVITY" >"$ATTRIBUTED_EVENTS" || true
...

{
  ...
  echo "background-owner-cli-policy: temporary-stored-always-restored"
  ...
  echo "attributed-owned-keychain-activity-events: $ATTRIBUTED_ACTIVITY_COUNT"
